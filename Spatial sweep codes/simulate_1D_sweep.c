...
// run by typing : gcc simulate_1D_sweep.c -o plain_sweep -lgsl -lm
...
int main(int argc, char  *argv[]) {
    ...
    sprintf(outfile1, "L=%u_N=%u_s=%f_m=%f_%d", L, N, s, mig, m);
    ...
    for (m = 0; m < 100; m++) {
        sprintf(outfile, "L=%u_N=%u_s=%f_m=%f_%d", L, N, s, mig, m);
