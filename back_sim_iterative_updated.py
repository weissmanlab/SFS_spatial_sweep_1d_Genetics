...
def writeOutput(position, mutationCount):
    outputFileName = "output_back_sim.txt"
...
def recalcRecombination():
    ...
    genDistance = relDistance * r_per_base 
...
if __name__ == '__main__':
    ...
    while startIndx < endIndx:
        ...
        if abs(mutationSite - startIndx) * r_per_base >= 10 * s and implicit_calculated == False:
            ...
            totalMutationRate = endIndx * perBaseMutation #endIndx is the total number of simulated loci
            ...
            for i in range(1, nbase):
                ...
                mutations[expectedValue] = i #[Key, value] where key is expected number of loci and value is number of mutations
            ...
            for mutationCount in mutations:
                for _ in range(int(mutationCount)):
                    target = np.random.randint(0, len(validLoci) - 1)
                    writeOutput(validLoci[target], int(mutations[mutationCount]))
