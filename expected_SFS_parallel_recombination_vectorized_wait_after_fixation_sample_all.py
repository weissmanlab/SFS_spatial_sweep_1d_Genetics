...
def runner(idx):
    ...
    while t_after_fix < T_after_fix:
        ...
        # expected SFS = leaf counts * branch len (= 1 gen)
...
#                                       bins = np.arange(1, n + 2))
    ...
    while left_individuals > 1 and extra_gen < L ** 2 / m:
...
#                                       bins = np.arange(1, n + 2))
    ...
    if np.mod(idx, 1000) == 0:
        np.savetxt('expected_SFS_L={}_N={}_s={:.6f}_m={:.6f}_r={:.6f}_tfinal={}_nsample={}_tfix={}_sample_uniform_{}.txt'.format(L,
           N, s, m, r, tfinal, n, T_after_fix, idx), SFS)
    return SFS, H
...
if __name__ == '__main__':
    ...
    SFS /= N_SFS
    np.savetxt('expected_SFS_L={}_N={}_s={:.6f}_m={:.6f}_r={:.6f}_tfinal={}_nsample={}_tfix={}_sample_uniform_navg={}.txt'.format(L,
                N, s, m, r, tfinal, nbase, T_after_fix, N_SFS), SFS)
    np.savetxt('expected_H_L={}_N={}_s={:.6f}_m={:.6f}_r={:.6f}_tfinal={}_nsample={}_tfix={}_sample_uniform_navg={}.txt'.format(L,
                N, s, m, r, tfinal, nbase, T_after_fix, N_SFS), H_items)
